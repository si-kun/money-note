    ...
    <div className="flex flex-col h-full max-h-full flex-1 gap-3 py-1 overflow-y-auto">
      {dailyIncome?.map((income) => (
        <Card className="bg-blue-200" key={income.id}>
          <CardContent className="flex items-center px-3 lg:px-6">
            <span className="w-[30%] overflow-hidden whitespace-nowrap truncate">【{income.category.name}】</span>
            <span className="w-[40%] overflow-hidden whitespace-nowrap truncate">{income.title || ""}</span>
      ...
      {dailyPayment?.map((payment) => (
        <Card className="bg-red-200" key={payment.id}>
          <CardContent className="flex items-center px-3 lg:px-6">
            <span className="w-[30%] truncate">【{payment.category.name}】</span>
            <span className="w-[40%] truncate">{payment.title || ""}</span>
